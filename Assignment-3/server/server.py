    ...
    while episodeNum < args.numEpisodes:
        ...
        if action == 'info':
            ...
            if event in ['goal', 'terminated']:
                episodeNum += 1
                if not verbose and episodeNum % max((args.numEpisodes//50),1) == 0:
            ...
            print('\n\nInvalid action! Terminating'+action, file=sys.stderr)
            break
    connection.sendall('TERMINATE\n'.encode('utf-8'))

    #print('\nReward for last 100 episodes:')
    for i in episodeRewards:
        print(str(i)+",",end='')
    print("\nCumulative Reward:"+str(sum(episodeRewards)))
